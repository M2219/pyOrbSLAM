...
class Tracking:
    ...
    def grab_image_stereo(self, mImGray, imGrayRight, timestamp):
        ...
        self.mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, self.mpORBExtractorLeft, self.mpORBExtractorRight, self.mpORBVocabulary,
                ...
                if not self.mbOnlyTracking:
                    if self.mState == "OK":
                        ...
                        if not self.mVelocity or self.mCurrentFrame.mnId < self.mnLastRelocFrameId + 2:
                            bOK = self.track_reference_keyframe()
                        else:
